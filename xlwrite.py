...
def output(filename, sheet,num, name, present):
    my_file = Path('Attendance_Sheets/'+filename+str(datetime.now().date())+'.xls');
    if my_file.is_file():
        rb = open_workbook('Attendance_Sheets/'+filename+str(datetime.now().date())+'.xls');
        book = copy(rb);
        sh = book.get_sheet(0)
        # file exists
    else:
    ...
    book.save('Attendance_Sheets/'+fullname)
    return fullname;
